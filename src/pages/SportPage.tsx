...
export function SportPage({ sport }: { sport: Sport }) {
  ...
  return <main className="sport-app" data-sport={sport.id} style={{ '--accent': sport.accent } as React.CSSProperties}>
    ...
    <section className="dashboard" id={sport.id}>
      ...
      {currentGameUnavailable && view !== 'rankings' ? <div className="empty-state"><b>Current game roster coming soon</b><span>Current rankings remain available, or choose All-Time Edition to play.</span></div> : view === 'quiz' ? <SportsQuiz sport={editionSport} /> : view === 'draft' ? <DreamTeamDraft key={`${sport.id}-${edition}`} sport={editionSport} /> : <><div className="tabs" role="tablist" aria-label="Rankings type"><button className={tab === 'teams' ? 'active' : ''} onClick={() => setTab('teams')}>{organisations ? 'Top 5 organisations' : 'Top 5 teams'}</button><button className={tab === 'players' ? 'active' : ''} onClick={() => setTab('players')}>Top 5 players</button></div><Rankings sport={editionSport} type={tab} /></>}
