...
type View = 'rankings' | 'quiz' | 'draft' | 'matches';
...
export function SportPage({ sport }: { sport: Sport }) {
  ...
  return <main className="sport-app" data-sport={sport.id} style={{ '--accent': sport.accent } as React.CSSProperties}>
    <a className="sport-home-link" href="/">&larr; Back to all sports</a>
    ...
    <section className="dashboard" id={sport.id}>
      ...
      <div className="section-heading"><div><p className="eyebrow">{edition === 'current' ? 'CURRENT FORM LEADERBOARD' : 'ALL-TIME RECORD BOOK'}</p><h2>{sport.icon} {sport.name} hub</h2><p>{edition === 'current' ? 'Ranked by recent performance, current form and the latest major competitions.' : 'Ranked by major titles, championships, medals and official records.'}</p></div>
        <div className="game-actions">{sport.id === 'football' && <button className="quiz-button" onClick={() => setView(view === 'matches' ? 'rankings' : 'matches')}>{view === 'matches' ? 'Close matches' : 'Fixtures & Results'}</button>}<button className="quiz-button" onClick={() => setView(view === 'quiz' ? 'rankings' : 'quiz')}>{view === 'quiz' ? 'Close quiz' : 'Quiz'}</button><button className="quiz-button draft-button" onClick={() => setView(view === 'draft' ? 'rankings' : 'draft')}>{view === 'draft' ? 'Close draft' : 'Dream Team Draft'}</button></div>
      </div>
      {view === 'matches' ? <FootballMatches /> : currentGameUnavailable && view !== 'rankings' ? <div className="empty-state"><b>Current game roster coming soon</b><span>Current rankings remain available, or choose All-Time Edition to play.</span></div> : view === 'quiz' ? <SportsQuiz sport={editionSport} /> : view === 'draft' ? <DreamTeamDraft key={`${sport.id}-${edition}`} sport={editionSport} /> : <><div className="tabs" role="tablist" aria-label="Rankings type"><button className={tab === 'teams' ? 'active' : ''} onClick={() => setTab('teams')}>{organisations ? 'Top 10 organisations' : 'Top teams'}</button><button className={tab === 'players' ? 'active' : ''} onClick={() => setTab('players')}>{edition === 'current' ? 'Current players' : 'Top 10 players'}</button></div><Rankings sport={editionSport} type={tab} edition={edition} /></>}
